    ...
    <div class="grid grid-cols-12 gap-8">
      ...
      <div class="col-span-12 lg:col-span-4 flex flex-col gap-6">
        <div class="glass-card rounded-2xl p-8 flex flex-col items-center text-center bg-white dark:bg-white/5 border border-slate-200 dark:border-white/5 shadow-xl dark:shadow-none transition-colors">
          ...
          <h2 class="text-2xl font-bold text-slate-900 dark:text-white font-['Space_Grotesk'] leading-tight mb-2 transition-colors">{{ orgData?.name || 'Unnamed Organization' }}</h2>
          ...
          <div class="space-y-4">
            <div class="flex items-start gap-4">
              ...
              <div class="text-left">
                ...
                <p class="text-xs text-slate-700 dark:text-white transition-colors">{{ orgData?.created_at ? new Date(orgData.created_at).toLocaleDateString() : 'Pending' }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="glass-card rounded-2xl p-8 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/5 shadow-xl dark:shadow-none transition-colors">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div v-for="field in [
              ...
              <div class="bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm text-slate-700 dark:text-white/80 font-medium transition-colors">{{ field.value || 'N/A' }}</div>
            </div>
            <div class="md:col-span-2 flex flex-col gap-2">
              ...
              <div class="bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm text-slate-600 dark:text-white/60 leading-relaxed min-h-[100px] transition-colors">{{ orgData?.description || 'No description defined.' }}</div>
            </div>
            <div class="md:col-span-2 flex flex-col gap-2">
              ...
              <div class="bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-sm text-slate-600 dark:text-white/60 transition-colors">{{ orgData?.address || 'Decentralized Operation' }}</div>
            </div>
          </div>
        </div>
      </div>
